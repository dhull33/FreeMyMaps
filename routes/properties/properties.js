...
app.param('property', (req, res, next, propertyID) => {
  if (req.user) {
    ...
    return db
      .any('SELECT * FROM properties WHERE property_id=$1 AND user_id=$2 AND active = true', [
        sanitizedPropId,
        req.user.user_id
      ])
      .then((property) => {
...
app.get('/properties/:property', (req, res) => {
  let firstName;
  if (req.user) {
...
app.post('/save/:property/map', (req, res, next) => {
  ...
  return db
    .none(
      'UPDATE properties SET map_features=$1, map_center=$2 WHERE' +
...
app.get('/properties/render/:property', (req, res, next) => {
